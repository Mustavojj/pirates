const translations = {
    en: {
        ...
        min_withdraw_dogs_amount: "Minimum Withdrawal: 200 DOGS",
        ...
        promote_step1: "Add @DogsPirateBot as admin on your channel",
    ...
    ar: {
        ...
        min_withdraw_dogs_amount: "الحد الأدنى للسحب: 200 DOGS",
        ...
        promote_step1: "أضف @DogsPirateBot كمدير في قناتك",
    ...
    ru: {
        ...
        min_withdraw_dogs_amount: "Минимальный вывод: 200 DOGS",
        ...
        promote_step1: "Добавьте @DogsPirateBot как администратора в ваш канал",
    ...
    fa: {
        ...
        min_withdraw_dogs_amount: "حداقل برداشت: ۲۰۰ DOGS",
        ...
        promote_step1: "@DogsPirateBot را به عنوان مدیر به کانال خود اضافه کنید",
...
class App {
    ...
    updateHeaderBalances() {
        ...
        const dogsHeader = document.getElementById('header-dogs');
        if (powerHeader) powerHeader.innerHTML = `<i class="fas fa-bolt"></i> ${this.formatNumber(Math.floor(this.powerBalance))}`;
        if (dogsHeader) dogsHeader.innerHTML = `<img src="${this.config.DOGS_ICON}" style="width:14px;height:14px;border-radius:50%;object-fit:cover;"> ${this.formatDogs(this.dogsBalance)}`;
    }
    ...
    async loadUserData() {
        ...
        try {
            ...
            this.referralDogsEarnings = user.referral_dogs_earnings || 0;
            ...
            this.pendingDogsReward = user.pending_dogs_reward || 0;
    ...
    async saveUserData(immediate = false) {
        ...
        try {
            ...
            if (this._dirtyDogs) updates.dogsBalance = this.dogsBalance;
        ...
        try {
            ...
            if (result.user) {
                if (type === 'power') {
                    ...
                    this.dogsBalance = result.user.dogs_balance || 0;
                    this.referralDogsEarnings = result.user.referral_dogs_earnings || 0;
                }
                this.userLevel = result.user.level || 1;
                this.updateLevelFromPower();
                this.updateHeaderBalances();
                const typeName = type === 'power' ? 'Power' : 'DOGS';
    ...
    async claimMiningRewards() {
        ...
        if (result.user) {
            ...
            this.updateHeaderBalances();
            this.renderMining();
            this.showNotification('Reward Claimed', `You have received ${result.claimed.toFixed(3)} DOGS`, 'success');
            this.vibrate('success');
        }
    }
            ...
            <div class="modal-content blue-modal">
                ...
                <div class="modal-body">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="value">${amount} DOGS</span>
                    </div>
                    <div class="detail-row">
                        <span class="label">${this.t('fees_label')}</span>
                        <span class="value fees">${fees} DOGS</span>
                    </div>
                    <div class="detail-row">
                        <span class="label">${this.t('received_amount')}</span>
                        <span class="value received">${received} DOGS</span>
                    </div>
                    <div class="confirm-note">${this.t('are_you_sure')}</div>
                    <button id="confirm-withdrawal-btn" class="confirm-btn blue-btn">${this.t('confirm')}</button>
    ...
    renderMining() {
        ...
        const hourlyRate = this.getHourlyDogsRate();
        const dailyRate = this.getDailyDogsRate();
        const monthlyRate = this.getMonthlyDogsRate();
        const progressPercent = this.getMiningProgressPercent();
        const radius = 55;
        const circumference = 2 * Math.PI * radius;
        const dashOffset = circumference - (progressPercent / 100) * circumference;

        const showStartButton = !this.miningActive && this.pendingDogsReward <= 0;
        const showClaimButton = !this.miningActive && this.pendingDogsReward > 0;
        ...
        el.innerHTML = `
            <div class="mining-card blue-card">
                <div class="mining-icon-container">
                    ...
                    <svg class="progress-ring" width="120" height="120" viewBox="0 0 120 120">
                        <circle cx="60" cy="60" r="${radius}" fill="none" stroke="rgba(59, 130, 246, 0.2)" stroke-width="5"/>
                        <circle class="progress-ring-circle" cx="60" cy="60" r="${radius}" fill="none" stroke="#3B82F6" stroke-width="5" stroke-dasharray="${circumference}" stroke-dashoffset="${circumference}" stroke-linecap="round" transform="rotate(-90 60 60)"/>
                    </svg>
                </div>
                <h3>${this.t('mining_rig')}${this.userLevel}</h3>
                <div class="rate-stats blue-stats">
                ...
                ${showMiningActive ? `
                    ...
                    <div class="mining-active-badge blue-badge"><i class="fas fa-circle"></i> ${this.t('mining_active')}</div>
                ` : ''}
                ${showStartButton ? `<button id="start-mining-btn" class="mining-action-btn blue-btn"><i class="fas fa-play"></i> ${this.t('start_mining')}</button>` : ''}
                ${showClaimButton ? `<button id="claim-mining-btn" class="mining-claim-btn blue-btn"><i class="fas fa-gift"></i> ${claimText}</button>` : ''}
            </div>

            ${!welcomeBonusClaimed ? `
            <div class="quest-card blue-quest">
                <div class="quest-header">
                    <div class="quest-icon blue-icon"><i class="fas fa-gift"></i></div>
                    ...
                    <button id="claim-welcome-quest" class="quest-claim-btn blue-btn"><i class="fas fa-gift"></i> ${this.t('claim_quest')}</button>
                </div>
            </div>
            ` : ''}

            <div class="boost-card blue-card">
                ...
                <div class="boost-input-group">
                    <input type="number" id="boost-amount" class="form-input blue-input" placeholder="${this.t('enter_dogs')}" min="1" step="1">
                    <button id="boost-btn" class="boost-btn blue-btn">${this.t('convert')}</button>
                </div>
            ...
            <div class="ad-card blue-card">
                ...
                <button id="watch-ad-btn" class="ad-btn blue-btn" ${!adsgramAvailable ? 'disabled' : ''}>
            ...
            <div class="quest-card blue-quest">
                <div class="quest-header">
                    <div class="quest-icon blue-icon"><i class="fas fa-trophy"></i></div>
                    <div class="quest-info">
                    ...
                    ${levelQuestComplete ? `<button id="claim-level-quest" class="quest-claim-btn blue-btn"><i class="fas fa-trophy"></i> ${this.t('claim_quest')}</button>` : `<button class="quest-claim-btn locked" disabled><i class="fas fa-lock"></i> Locked</button>`}
                </div>
            </div>
            ` : ''}
            ...
            <div class="quest-card blue-quest">
                <div class="quest-header">
                    <div class="quest-icon blue-icon"><i class="fas fa-clipboard-list"></i></div>
                    <div class="quest-info">
                    ...
                    ${taskQuestComplete ? `<button id="claim-task-quest" class="quest-claim-btn blue-btn"><i class="fas fa-clipboard-list"></i> ${this.t('claim_quest')}</button>` : `<button class="quest-claim-btn locked" disabled><i class="fas fa-lock"></i> Locked</button>`}
                </div>
            </div>
            ` : ''}
            ...
            <div class="quest-card blue-quest">
                <div class="quest-header">
                    <div class="quest-icon blue-icon"><i class="fas fa-users"></i></div>
                    <div class="quest-info">
                    ...
                    ${referralQuestComplete ? `<button id="claim-referral-quest" class="quest-claim-btn blue-btn"><i class="fas fa-users"></i> ${this.t('claim_quest')}</button>` : `<button class="quest-claim-btn locked" disabled><i class="fas fa-lock"></i> Locked</button>`}
                </div>
            </div>
            ` : ''}
...
<div class="community-card blue-card">
    <div class="community-header">
        <div class="community-icon blue-icon"><i class="fas fa-bullhorn"></i></div>
...
<div class="community-card blue-card">
    <div class="community-header">
        <div class="community-icon blue-icon"><i class="fas fa-money-bill-wave"></i></div>
        <div class="community-info">
...
<div class="community-card blue-card">
    <div class="community-header">
        <div class="community-icon blue-icon"><i class="fas fa-tasks"></i></div>
        <div class="community-info">
        ...
        document.getElementById('boost-btn')?.addEventListener('click', async () => {
            ...
            if (amount > this.dogsBalance) {
                this.showNotification('Error', 'Insufficient DOGS balance', 'error');
                this.vibrate('error');
                return;
            }
            ...
            btn.disabled = true;
            btn.innerHTML = '<i class="fas fa-spinner fa-pulse"></i>';
            await this.convertDogsToPower(amount);
            btn.disabled = false;
                        ...
                        <div class="task-header">
                            ...
                            <div class="task-info">
                                ...
                                <div class="task-reward">
                                    ...
                                    <span class="reward-badge"><img src="${this.config.DOGS_ICON}" style="width:14px;height:14px;border-radius:50%;object-fit:cover;"> ${dogsReward}</span>
                                </div>
    ...
    renderTeam() {
        ...
        document.getElementById('claim-dogs-earnings')?.addEventListener('click', async () => {
            const btn = document.getElementById('claim-dogs-earnings');
            btn.disabled = true;
            btn.innerHTML = '<i class="fas fa-spinner fa-pulse"></i>';
            await this.claimReferralEarnings('dogs');
            btn.disabled = false;
            btn.innerHTML = this.hasPromotionBonus ? this.t('claim_with_bonus') : this.t('claim_default');
            this.renderTeam();
            this.vibrate('success');
    ...
    renderWallet() {
        ...
        if (!this.userWallet) {
            walletSetupHtml = `
            <div class="wallet-setup-card blue-card">
                <div class="setup-title"><i class="fas fa-wallet"></i> ${this.t('setup_wallet')}</div>
                <div class="setup-input-group">
                    <input type="text" id="wallet-setup-input" class="form-input blue-input" placeholder="${this.t('wallet_address')}">
                    <button id="wallet-setup-btn" class="setup-btn blue-btn">${this.t('confirm_wallet')}</button>
                </div>
        ...
        el.innerHTML = `
            <div class="wallet-card blue-card">
                <div class="wallet-balances">
                    <div class="wallet-balance-item">
                        <div class="wallet-balance-icon"><img src="${this.config.DOGS_ICON}" style="width:28px;height:28px;border-radius:50%;"></div>
                        <div class="wallet-balance-label">${this.t('dogs_balance')}</div>
                        <div class="wallet-balance-amount">${this.formatDogs(this.dogsBalance)}</div>
                    </div>
                    <div class="wallet-balance-item">
                        <div class="wallet-balance-icon"><i class="fas fa-bolt" style="color:var(--blue);font-size:1.5rem;"></i></div>
            ...
            <div class="withdraw-card blue-card">
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <input type="number" id="withdraw-amount" class="form-input blue-input" placeholder="${this.t('min_withdraw_dogs_amount')}" step="1">
    ...
    saveSettings() {
        localStorage.setItem('dogs_pirates_settings', JSON.stringify({ lang: this.lang }));
    }

    loadSettings() {
        const saved = localStorage.getItem('dogs_pirates_settings');
        if (saved) {
        ...
        try {
            ...
            const headerHtml = `
                <div class="header-balances" id="header-balances">
                    ...
                    <div class="header-balance" id="header-dogs"><img src="${this.config.DOGS_ICON}" style="width:14px;height:14px;border-radius:50%;object-fit:cover;"> ${this.formatDogs(this.dogsBalance)}</div>
                </div>
            `;
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('.lang-option').forEach(opt => {
            opt.addEventListener('click', () => {
                ...
                localStorage.setItem('dogs_pirates_lang', this.lang);
                const settings = JSON.parse(localStorage.getItem('dogs_pirates_settings') || '{}');
                settings.lang = this.lang;
                localStorage.setItem('dogs_pirates_settings', JSON.stringify(settings));
...
document.addEventListener('DOMContentLoaded', () => {
    if (!window.Telegram?.WebApp) {
        document.body.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#0A0A0A;color:#FFFFFF">Open from Telegram Mini App</div>';
        return;
    }
    window.app = new App();
